...
public class ShuMeiActivity extends AbstractJni implements IOResolver<AndroidFileIO> {
    private final AndroidEmulator emulator;
    private final VM vm;
    private final Module module;

    private ShuMeiActivity() {
...
//                .build();
        ...
        emulator = androidEmulatorBuilder.setRootDir(new File("/tmp/rootfs/default")).build();
        Memory memory = emulator.getMemory();
        LibraryResolver resolver = new AndroidResolver(23);
        memory.setLibraryResolver(resolver);
        emulator.getSyscallHandler().addIOResolver(this);
        vm = emulator.createDalvikVM(new File("unidbg-android/src/test/java/com/phil/shumei/com.jiuwu_1.25.0_1002500.apk"));
        vm.setJni(this);
        vm.setVerbose(true);
        DalvikModule dm = vm.loadLibrary(new File("unidbg-android/src/test/java/com/phil/shumei/libsmsdk.so"), true);
        module = dm.getModule();
...
//        Logger.getLogger(ARM32SyscallHandler.class).setLevel(Level.DEBUG);
    ...
    public static void main(String[] args) {
        ...
        ShuMeiActivity shuMeiActivity = new ShuMeiActivity();
    ...
    @Override
    public DvmObject<?> callStaticObjectMethodV(BaseVM vm, com.github.unidbg.linux.android.dvm.DvmClass dvmClass, String signature, com.github.unidbg.linux.android.dvm.VaList vaList) {
        ...
        } else if (signature.equals("javax/crypto/Cipher->getInstance(Ljava/lang/String;)Ljavax/crypto/Cipher;")) {
            return vm.resolveClass("javax/crypto/Cipher").newObject(signature);
        }
    ...
    public DvmObject<?> callObjectMethodV(BaseVM vm, DvmObject<?> dvmObject, String signature, VaList vaList) {
        ...
        } else if (signature.equals("java/security/cert/Certificate->getPublicKey()Ljava/security/PublicKey;")){
            return vm.resolveClass("java/security/PublicKey").newObject(signature);
        } else if (signature.equals("javax/crypto/Cipher->doFinal([B)[B")) {
    ...
    public DvmObject<?> getStaticObjectField(BaseVM vm, DvmClass dvmClass, String signature) {
        if (signature.equals("java/security/spec/MGF1ParameterSpec->SHA256:Ljava/security/spec/MGF1ParameterSpec;")){
            return vm.resolveClass("java/security/spec/MGF1ParameterSpec").newObject(signature);
        } else if (signature.equals("javax/crypto/spec/PSource$PSpecified->DEFAULT:Ljavax/crypto/spec/PSource$PSpecified;")) {
    ...
    public DvmObject<?> newObjectV(BaseVM vm, DvmClass dvmClass, String signature, VaList vaList) {
        if (signature.equals("javax/crypto/spec/OAEPParameterSpec-><init>(Ljava/lang/String;Ljava/lang/String;Ljava/security/spec/AlgorithmParameterSpec;Ljavax/crypto/spec/PSource;)V")) {
    ...
    public void callVoidMethodV(BaseVM vm, DvmObject<?> dvmObject, String signature, VaList vaList) {
        if (signature.equals("javax/crypto/Cipher->init(ILjava/security/Key;Ljava/security/spec/AlgorithmParameterSpec;)V")) {
